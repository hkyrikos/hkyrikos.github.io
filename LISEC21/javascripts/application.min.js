...
var watchlist_count = [];    // amounts for watchlist
...
// Zwischenspeicher id für dblClick
 ...
 * loadCatalogs()
 *
 * Diese Funktion lädt die verfügbaren Kataloge aus catalogs.xml und zeigt
...
function refreshImg() {
    ...
    if ($("#catalogs_div").height() > $(window.document).height() / 2) {
 ...
 * setCatalog(id, name)
 *
 * Lädt den Katalog mit der übergebenen ID
 *
 * @param id Die ID des Kataloges aus catalogs.xml. Diese ist dann der Dateiname des zu öffnenden Kataloges.
...
function parseCatalog(xml) {
  ...
  $(xml).find("stk").each(function () {
      ...
      li_string = "<li id=\"" + id + "\" data-materialnr=\"" + materialnr + "\" data-description=\"" + description + "\"><span class=\"item\">&nbsp;" + pos + name + "&nbsp;|&nbsp;" + materialnr + "</span>";
      ...
      if (level > oldLevel) {
          ...
          // Level ist höher - </ul> bis altes Level erreicht (z.B. von 1.1.1 auf 1.2)
...
$("#wlItems").html("(" + watchlist.length + " " + itm + ")");
if (jQuery.inArray(id, watchlist) > -1)
    $("#wlCounter").html(watchlist_count[jQuery.inArray(id, watchlist)]);
else
    $("#wlCounter").html("0");
...
$("#bWLAdd").unbind("dblclick");
  ...
  function bWLAdd_Event() {
      if (jQuery.inArray(idAfterClick, watchlist) == -1) {
          log(idAfterClick + " added to watchlist");
          watchlist.push(idAfterClick);
          watchlist_count.push(1);
          // send to customer-interface
          //watchlist_updated();
      }
      else {
          watchlist_count[jQuery.inArray(idAfterClick, watchlist)] += 1;
      }
      var itm = watchlist.length == 1 ? "Item" : "Items";
      ...
      $("#wlItems").html("(" + watchlist.length + " " + itm + ")");
      $("#wlCounter").html(watchlist_count[jQuery.inArray(idAfterClick, watchlist)]);
  ...
  function bWLRem_Event() {
      if (jQuery.inArray(idAfterClick, watchlist) > -1) {
          if (watchlist_count[jQuery.inArray(idAfterClick, watchlist)] == 1) {
              watchlist_count.splice(jQuery.inArray(idAfterClick, watchlist), 1);
              watchlist.splice(jQuery.inArray(idAfterClick, watchlist), 1);
          }
          else
              watchlist_count[jQuery.inArray(idAfterClick, watchlist)] -= 1;
      }
  ...
  $(xml_catalog).find("#" + id).each(function () {
      ...
      $(this).find("map").each(function () {
          ...
          $("#map_" + map_count).click(function () {
              ...
              // TODO: make this better because if the ul is only set to visible the plus/minus symbols are not changing!
              $("#tree").find("li[id='" + id + "']").parent().css("display", "block");
...
function resize() {
  ...
  //alert($(window.document).width() - $("#left").width() - 5 + "px");
  //alert($("#catalogs_div").height() + "-" + $(window.document).height());
}
